...
The Celery application is defined in `image_processing_service/celery.py`. Unlike other components, Celery does NOT read configuration from the project's Settings class. Instead, Celery must be configured using environment variables.
...
```python
from image_processing_service.celery import celery
...
```python
from image_processing_service.services.db import get_engine, AsyncSession
...
celery.conf.beat_schedule = {
    "cleanup-old-data": {
        "task": "image_processing_service.tasks.cleanup_old_data",
        "schedule": crontab(hour=2, minute=0),  # Run daily at 2 AM
    },
    "send-weekly-report": {
        "task": "image_processing_service.tasks.send_weekly_report",
        "schedule": crontab(day_of_week="monday", hour=9, minute=0),
    },
    "check-status-every-5-min": {
        "task": "image_processing_service.tasks.check_status",
...
```python
# In image_processing_service/celery.py
from image_processing_service.tasks import email, reports, cleanup
```
...
```bash
# Basic worker
celery -A image_processing_service.celery worker --loglevel=info

# With concurrency limit
celery -A image_processing_service.celery worker --loglevel=info --concurrency=4

# With specific queues
celery -A image_processing_service.celery worker --loglevel=info -Q celery,emails
```
...
celery -A image_processing_service.celery beat --loglevel=info

# Or combine worker and beat (NOT recommended for production)
celery -A image_processing_service.celery worker --beat --loglevel=info
```
...
celery -A image_processing_service.celery worker \
    --loglevel=info \
    --concurrency=8 \
    --pool=prefork
...
```bash
# List active tasks
celery -A image_processing_service.celery inspect active

# List scheduled tasks (ETA tasks)
celery -A image_processing_service.celery inspect scheduled

# List registered tasks
celery -A image_processing_service.celery inspect registered

# Worker statistics
celery -A image_processing_service.celery inspect stats

# Ping workers
celery -A image_processing_service.celery inspect ping
```
...
def test_periodic_tasks_registered():
    ...
    from image_processing_service.celery import setup_periodic_tasks
...
def test_cache_setup_handler_exists():
    ...
    from image_processing_service.celery import setup_caches
...
def test_cache_setup_imports():
    ...
    from image_processing_service.services.cache import configure_caches
...
def test_task_with_database(monkeypatch):
    ...
    with patch("image_processing_service.services.db.get_session", return_value=mock_session):
...
export CELERY_BROKER_URL="redis://localhost:6379/0"
celery -A image_processing_service.celery worker --loglevel=debug --pool=solo
```
