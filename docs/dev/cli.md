...
The CLI application is defined in `image_processing_service/cli.py` and automatically configured as an entry point in `pyproject.toml`:

```toml
[project.scripts]
image_processing_service = "image_processing_service.cli:app"
```

After installation, the CLI is available as the `image_processing_service` command.
...
```bash
image_processing_service command-name --help
```
...
```bash
image_processing_service my-command --option value argument
```
...
Add a basic command to `image_processing_service/cli.py`:
...
```bash
image_processing_service hello "World"
# Output: Hello, World!
```
...
```bash
image_processing_service process input.txt --output output.txt --verbose
image_processing_service process input.txt -o output.txt -v
```
...
```bash
image_processing_service export --count 50 --format yaml --output data.yaml
```
...
```python
from image_processing_service.cli import syncify
import httpx
...
```python
from image_processing_service.services.db import get_engine
...
```bash
image_processing_service db migrate
image_processing_service db seed
image_processing_service user create john john@example.com
image_processing_service user list
```
...
```python
from image_processing_service.commands import database, users
...
from image_processing_service.cli import app, syncify
import asyncio
...
def test_version_command_runs():
    ...
    assert "image_processing_service" in result.stdout.lower()
...
# Run commands
image_processing_service --help
image_processing_service my-command
```
...
python -m image_processing_service.cli --help
```
