...
ENV APP_MODULE=image_processing_service.www:app
...
ENV APP_MODULE=image_processing_service.celery:celery
...
```yaml
volumes:
  - "./image_processing_service:/app/image_processing_service" # Source code
...
docker build -f dockerfile.www -t image_processing_service-www:latest .

# Tag for registry
docker tag image_processing_service-www:latest ghcr.io/your-org/image_processing_service-www:latest

# Push to registry
docker push ghcr.io/your-org/image_processing_service-www:latest
```
...
if [ ! -z "$CREATE_TEST_DATA" ]; then
  ...
  python -m image_processing_service.cli test-data
fi
...
services:
  www:
    image: ghcr.io/your-org/image_processing_service-www:latest
    restart: always
...
# Run a command
docker-compose exec www python -m image_processing_service.cli version

# Check database connection
docker-compose exec www python -c "from image_processing_service.services.db import engine; print(engine)"
```
