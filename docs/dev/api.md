...
The FastAPI application is defined in `image_processing_service/www.py` and includes:

- **Automatic API documentation** at `/docs` (Swagger UI) and `/redoc` (ReDoc)
- **Static file serving** from `image_processing_service/static/` via the `/static/` endpoint
...
Create a new route in `image_processing_service/www.py`:
...
from image_processing_service.services.db import get_session_depends
...
Static files are served from `image_processing_service/static/`:

1. **Add files** to the `static/` directory:

   ```
   image_processing_service/static/
   ├── css/
   │   └── styles.css
   ├── js/
   │   └── app.js
...
from image_processing_service.www import app
...
uvicorn image_processing_service.www:app --reload --host 0.0.0.0 --port 8000
...
uvicorn image_processing_service.www:app --host 0.0.0.0 --port 8000 --workers 4

# Or using gunicorn with uvicorn workers
gunicorn image_processing_service.www:app -w 4 -k uvicorn.workers.UvicornWorker
```
