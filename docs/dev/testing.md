...
pytest --cov=./image_processing_service --cov-report=term-missing tests

# Generate HTML coverage report
pytest --cov=./image_processing_service --cov-report=html tests
# Open htmlcov/index.html in your browser

# Show coverage for specific file
pytest --cov=./image_processing_service/services/cache.py tests/services/test_cache.py
```
...
def test_version_command():
    ...
    assert "image_processing_service" in result.stdout
```
...
```python
@pytest.mark.asyncio
@patch("image_processing_service.services.cache.get_cached")
...
def test_celery_task_registration():
    ...
    from image_processing_service.celery import celery, my_task

    assert "image_processing_service.celery.my_task" in celery.tasks


def test_celery_task_execution():
    """Test direct task execution."""
    from image_processing_service.celery import my_task
...
def test_settings_load():
    ...
    assert settings.project_name == "image_processing_service"
...
pytest --cov=./image_processing_service --cov-fail-under=80 tests
```
...
omit = [
  "./image_processing_service/_version.py",
]
```
